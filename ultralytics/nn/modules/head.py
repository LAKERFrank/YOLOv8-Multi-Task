...
class DetectV2(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):  # detection layer
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3), Conv(c2, c2, 3), nn.Conv2d(c2, self.feat_no * self.reg_max * self.num_groups, 1)) for x in ch)
        self.cv3 = nn.ModuleList(nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc * self.num_groups, 1)) for x in ch)
        self.cv4 = nn.ModuleList(
            nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc * 2 * self.num_groups, 1)) for x in ch)
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
...
class DetectWithHit(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):  # detection layer
        ...
        c2, c3 = max((16, ch[0] // 4, self.reg_max * 4)), max(ch[0], min(self.nc, 100))  # channels
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3), Conv(c2, c2, 3), nn.Conv2d(c2, 4*10, 1)) for x in ch)
        self.cv3 = nn.ModuleList(nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc*10, 1)) for x in ch)
        self.cv4 = nn.ModuleList(nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc*10, 1)) for x in ch)
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
...
class Pose(Detect):
    ...
    def forward(self, x):
        ...
        if not self.training and (self.anchors.numel() == 0 or self.shape != x[0].shape):
            # generate anchors only from first detection level to match outputs
            anchors, strides = make_anchors([x[0]], self.stride[:1], 0.5)
            self.anchors, self.strides = (a.transpose(0, 1) for a in (anchors, strides))
            self.shape = x[0].shape
        kpt = self.cv4[0](x[0]).view(bs, self.nk, -1)  # (bs, nk, h*w)
        x = self.detect(self, x)
        if self.training:
            return x, kpt
