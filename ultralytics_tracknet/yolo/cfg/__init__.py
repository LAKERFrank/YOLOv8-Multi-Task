...
from ultralytics_tracknet.yolo.utils import (DEFAULT_CFG, DEFAULT_CFG_DICT, DEFAULT_CFG_PATH, LOGGER, ROOT, USER_CONFIG_DIR,
...
def handle_yolo_hub(args: List[str]) -> None:
    ...
    """
    from ultralytics_tracknet import hub
...
def entrypoint(debug=''):
    ...
    if 'rtdetr' in model.lower():  # guess architecture
        from ultralytics_tracknet import RTDETR
        model = RTDETR(model)  # no task argument
    elif 'sam' in model.lower():
        from ultralytics_tracknet import SAM
        model = SAM(model)
    else:
